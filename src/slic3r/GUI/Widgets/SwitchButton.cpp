...
SwitchButton::SwitchButton(wxWindow* parent, wxWindowID id)
    ...
    , thumb_color(std::pair{accent_wx_color(), (int) StateColor::Checked}, std::pair{0xD9D9D9, (int) StateColor::Normal})
{
...
void SwitchBoard::doRender(wxDC &dc)
{
	...
	/*left*/
    if (switch_left) {
        is_enable ? dc.SetBrush(wxBrush(accent_wx_color())) : dc.SetBrush(disable_color);
	...
	/*right*/
    if (switch_right) {
        if (is_enable) {dc.SetBrush(wxBrush(accent_wx_color()));
        } else {dc.SetBrush(disable_color);}
